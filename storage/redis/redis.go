...
// TODO: error handling
func New(p Post) (e error) {

	key := fmt.Sprintf("post:" + p.ID)

	client.HSet(key, "title", p.Title)
	client.HSet(key, "content", string(p.Content))
	client.HSet(key, "created", fmt.Sprint(time.Now().Unix()))
	client.HSet(key, "modified", fmt.Sprint(time.Now().Unix()))
	client.HIncrBy(key, "accessed", 1)

	z := redis.Z{Score: float64(time.Now().Unix()), Member: p.Title}
	client.ZAdd("posts", z)
...
func GetRaw(id string, incr bool) (p Post, e error) {
